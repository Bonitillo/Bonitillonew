    ...
    if not 'list' in url:
        if not 'catid' in url:
            ...
            action= name[xbmcgui.Dialog().select('Please Choose Preference', GRRR)]
                 #type=popular&t=movie&catid=0&page=1&os=Android&v=291.0&k=0
            new_url = API+'type=%s&t=%s&catid=0&os=Android&v=4.10&k=%s' % (action,url,KID)+'&page='
        else:new_url=url   
    else:new_url=url
...
def Genre(url):
    new_url = API+'type=%s&catid=0&os=Android&v=4.10&k=%s' % (url,KID)
    ...
    for field in data:
        ...
        url=str(field['id'])
        URL='http://playboxhd.com/api/box?type=popular&t=movie&catid=%s&page=1&os=Android&v=4.10&page=' % url
        addDir(name,URL,1,'','')
...
def SEARCHPLAYBOX(url):
    ...
    new_url = API+'type=search&os=Android&v=4.10&k=0&keyword=%s' % search_entered
...
def TodayList(url):
    ...
    new_url = 'http://playboxhd.com/api/box?t=box&type=list&os=Android&v=4.10&k=%s' % KID
    try:link = json.loads(OPEN_URL(new_url))
    except:return PlayBoxFucker()
...
def TodayListGetContent(url):
    PAGE= 1
    new_url = 'http://playboxhd.com/api/box?type=detail&t=box&id=%s&os=Android&v=4.10&k=%s' % (url,KID)+'&page='
...
def GetDetail(name,url,iconimage):
    ...
    new_url = 'http://playboxhd.com/api/box?type=detail&id=%s&os=Android&v=4.10&k=%s' % (url,KID)
    ...
    if len(data)>1: 
        ...
        for field in data:
            ID=str(field['id'])
            continue
        new_url = 'http://playboxhd.com/api/box?type=stream&id=%s&os=Android&v=4.10&k=0' % ID
...
def GetStreamLinks(url,iconimage,name,page):
    NAME=page
    new_url = 'http://playboxhd.com/api/box?type=stream&id=%s&os=Android&v=4.10&k=0' % url
...
def OPEN_URLS(url):
    url=url+'&ui=%7B%22DeviceID%22%3A%22217cbf93dee7cd5e8e29cb2289609190%22%2C%22OsName%22%3A%22Android%22%2C%22OsVersion%22%3A%2216%22%2C%22AppName%22%3A%22PlayBox%22%2C%22AppVersion%22%3A%221.0.0%22%7D'

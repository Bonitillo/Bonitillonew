...
Q1BiYXNl = 'aHR0cDovL21yZXBvLnVrL21lcmxpbmFkZG9ucy9mb3Jwcm9qZWN0bS9ob21lLnhtbA=='.decode('base64')
Q1BiYXN2 = 'aHR0cDovL213aXouY28udWsvbWVybGluYWRkb25zL3NlYXJjaC9tb3ZpZXMudHh0'.decode('base64')
Q1BiYXN3 = 'aHR0cDovL213aXouY28udWsvbWVybGluYWRkb25zL3NlYXJjaC9tcDMudHh0'.decode('base64')

addon = xbmcaddon.Addon('plugin.video.projectm')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.projectm]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Project M,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Project M,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")

				
def CPIndex():
    addon_log("CPIndex")
    getData(Q1BiYXNl,'')
    addDir('[B][COLOR yellow]Search Movies[/B][/COLOR]',Q1BiYXN2,41,'http://mwiz.co.uk/merlinaddons/icons/search.png' ,  FANART,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Project M,New source added.,5000,"+icon+")")
        if not url is None:
...
def Search_input(url):
    Dialog = xbmcgui.Dialog()
    Search_title = Dialog.input('SEARCH - Item Not Found? Then Look Manually!', type=xbmcgui.INPUT_ALPHANUM)
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('Project M - Select A Source', List)
         if rNo>=0:
             rName=str(List[rNo])
             rURL=str(ListU[rNo])
             try:
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Project M,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        xbmc.executebuiltin("XBMC.Notification(Project M, Attempting To Load Video... - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('Project M','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Project M', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from Project M Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
            if not name in FAV:
                contextMenu.append(('Add to Project M Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        headers = dict(headers = {'User-agent' : 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:19.0) Gecko/20100101 Firefox/19.0','Referer': 'http://joker.org/','Accept-Encoding':'gzip, deflate','Content-Type': 'application/json;charset=utf-8','Accept': 'application/json, text/plain, */*'})
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(Project M,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from Project M Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to Project M Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(Project M,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Project M,Please [COLOR blue]install Youtube-dl[/COLOR] module ,10000,"")")

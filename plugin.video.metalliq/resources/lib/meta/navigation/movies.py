...
from settings import CACHE_TTL, SETTING_MOVIES_LIBRARY_FOLDER, SETTING_MOVIES_PLAYED_BY_ADD, SETTING_FORCE_VIEW,\
    ...
    SETTING_MOVIES_DEFAULT_AUTO_ADD, SETTING_RANDOM_PAGES
...
def trakt_movies_search_term(term, page):
    ...
    return list_trakt_movie_items(results, pages, page)
...
def trakt_movies_played(page, raw=False):
    ...
    else: return list_trakt_movie_items_with_genres(results, pages, page)
...
def trakt_movies_popular(page, raw=False):
    ...
    if raw: return results
    else: return list_trakt_items_with_genres(results, pages, page)
...
def list_trakt_movies_popular_paginated(results, pages, page):
    ...
    if pages > page:
        items.append({
            ...
            'path': plugin.url_for("trakt_movies_popular", page=page + 1),
...
def trakt_movies_collection_to_library():
    from trakt import trakt
    if dialogs.yesno(_("Scan item to library"), "{0}[CR]{1}".format(_("Add %s") % ("'{0} {1} {2}'".format("Trakt", _("movie"), _("Collection").lower())),_("Are you sure?"))):
...
def trakt_movies_watchlist_to_library():
    from trakt import trakt
    if dialogs.yesno(_("Scan item to library"), "{0}[CR]{1}".format(_("Add %s") % ("'{0} {1} {2}'".format("Trakt", _("movie"), _("Watchlist").lower())),_("Are you sure?"))):
...
def list_trakt_movies_plain(results):
    from trakt import trakt
    genres_dict = dict([(x['slug'], x['name']) for x in trakt.trakt_get_genres("movies")])
    movies = [get_trakt_movie_metadata(item["movie"], genres_dict) for item in results]
    items = [make_movie_item(movie) for movie in movies]
...
def list_trakt_items_with_genres(results, pages, page):
    from trakt import trakt
    genres_dict = dict([(x['slug'], x['name']) for x in trakt.trakt_get_genres("movies")])
    movies = [get_trakt_movie_metadata(item, genres_dict) for item in results]
    items = [make_movie_item(movie) for movie in movies]

...
from settings import CACHE_TTL, SETTING_TV_LIBRARY_FOLDER, SETTING_TV_DEFAULT_AUTO_ADD, SETTING_TV_PLAYED_BY_ADD, SETTING_TV_DEFAULT_PLAYER_FROM_LIBRARY, SETTING_TV_BATCH_ADD_FILE_PATH, SETTING_MOVIES_BATCH_ADD_FILE_PATH, SETTING_FORCE_VIEW, SETTING_TVSHOWS_VIEW, SETTING_AIRED_UNKNOWN, SETTING_INCLUDE_SPECIALS
...
def trakt_tv_collection_to_library():
    from trakt import trakt
    if dialogs.yesno(_("Scan item to library"), "{0}[CR]{1}".format(_("Add %s") % ("'{0} {1} {2}'".format("Trakt", _("TV"), _("Collection").lower())),_("Are you sure?"))):
...
def trakt_tv_watchlist_to_library():
    from trakt import trakt
    if dialogs.yesno(_("Scan item to library"), "{0}[CR]{1}".format(_("Add %s") % ("'{0} {1} {2}'".format("Trakt", _("TV"), _("Watchlist").lower())),_("Are you sure?"))):
...
def tv_season(id, season_num):
    ...
    plugin.set_content('episodes')
    if FORCE == True: return plugin.finish(items=list_episodes_tvdb(id, season_num), sort_methods=SORT, view_mode=VIEW)
...
def make_tvshow_item(info):
    ...
    if info['poster'] == None or info['poster'] == "": info['poster'] = "https://raw.githubusercontent.com/OpenELEQ/Style/master/MetalliQ/default/unavailable_showposter.png"

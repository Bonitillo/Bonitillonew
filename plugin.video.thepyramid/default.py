...
Change_Log_Path = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.thepyramid/Change_Log_Temp'))
def Change_Log():
    if not os.path.exists(Change_Log_Path):
        TextBoxes('[COLORgreen]Update Succesful To V 1.41[/COLOR]','\n\n[COLORorange]CONGRATULATIONS The Pyramid Has Been Updated Successfully[/COLOR]\n\n[COLORblue]VISIT https://www.facebook.com/groups/thepyramidkodi/ for help and advice[/COLOR]\n\n[COLORblue]OR FOLLOW US ON TWITTER @TombBuild[/COLOR]\n\n[COLORred]Please Make Sure You Have Installed From An Official Source[/COLOR]')
...
def SKindex():
    Change_Log()
    addDir('[B][COLOR gold]THE PYRAMID FAVOURITES[/B][/COLOR]','[B][COLOR gold]THE PYRAMID FAVOURITES[/B][/COLOR]',4,'http://files.softicons.com/download/system-icons/junior-icons-by-treetog-artwork/png/256/Folder%20Favorites.png' ,  FANART,'','','','')
...
def Search_loop():
	...
	if Search_name == '':
		...
		if Search_title[0] in 'abcd':
			url_to_open = 'http://tombraiderbuilds.co.uk/addon/movies/A-D/A-D.txt'
		elif Search_title[0] in 'efgh':
			url_to_open = 'http://tombraiderbuilds.co.uk/addon/movies/E-H/E-H.txt'
		elif Search_title[0] in 'ijkl':
			url_to_open = 'http://tombraiderbuilds.co.uk/addon/movies/I-L/I-L.txt'
		elif Search_title[0] in 'mnop':
			url_to_open = 'http://tombraiderbuilds.co.uk/addon/movies/M-P/M-P.txt'
		elif Search_title[0] in 'qrs':
			url_to_open = 'http://tombraiderbuilds.co.uk/addon/movies/Q-S/Q-S.txt'
		elif Search_title[0] in 't':
			url_to_open = 'http://tombraiderbuilds.co.uk/addon/movies/T/T.txt'
		elif Search_title[0] in 'uvwxyz':
			url_to_open = 'http://tombraiderbuilds.co.uk/addon/movies/U-Z/U-Z.txt'
		elif Search_title[0] in '0123456789':
			url_to_open = 'http://tombraiderbuilds.co.uk/addon/movies/0-1000000/0-1000000.txt'
		...
		HTML2 = Open_Url('http://tombraiderbuilds.co.uk/addon/movies/New%20Releaes/newreleases.txt')
		...
		HTML3 = Open_Url('http://tombraiderbuilds.co.uk/addon/boxsets/boxsets.txt')
...
#                print channel
                ...
                try:
                    if name == '[B][COLORgold]YOU ARE IN THE MOVIE ZONE[/B][/COLOR]':
                        ...
                        addDir('[B][COLORgold]PYRAMID MOVIE SEARCH[/B][/COLOR]','[B][COLORgold]PYRAMID MOVIE SEARCH[/B][/COLOR]',41,'http://tombraiderbuilds.co.uk/Icons/search.jpg' ,  FANART,'','','','')
                    elif linkedUrl=='':
                        addDir(name.encode('utf-8', 'ignore'),url.encode('utf-8'),2,thumbnail,fanArt,desc,genre,date,credits,True)
                    else:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('[COLORblue]Select A Pyramid Source[/COLOR]', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
def urlsolver(url):
    ...
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification([COLORblue]The Pyramid[/COLOR] ,[COLORblue]Enjoy Your Viewing On The Pyramid[/COLOR] ,5000)")
        import urlresolver
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from The Pyramid Favourites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
            if not name in FAV:
                contextMenu.append(('Add to The Pyramid Favourites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from The Pyramid Favourites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to The Pyramid Favourites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
